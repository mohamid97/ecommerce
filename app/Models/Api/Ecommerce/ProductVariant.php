...
class ProductVariant extends Model implements TranslatableContract
{
    ...
    /**
     * Calculate the variant's own (manually set) discounted price.
     */
    public function calculateOwnDiscountPrice(): float
    {
        ...
        if ($this->discount_type === 'percentage') {
            return $salePrice - ($salePrice * ((float) ($this->discount_value ?? 0) / 100));
        }

        return max(0, $salePrice - (float) ($this->discount_value ?? 0));
    }

    /**
     * Return the best price after discount — whichever is lower between the
     * variant's own discount and the best active promotion discount.
     */
